...
(c) Copyright GLOBAL Travel Computer Services 1994, 1995, 1996, 1997, 1998, 1999,
                                              2000, 2001
                SID: 1.46
      Delta Created: 17/10/31 - 23:53:13
         Last Delta: app/src/common/InterfaceRecord/_us 10/31/17 - 23:53:13 (/TaskEnv/sw/app/src/common/InterfaceRecord/_us/s.MIR.h)
...
class CMIR : public CInterfaceRecord
{
   public:
      void createCMIR(CPNR &po_PNR)  ;
      void printAll() ;
      CMIR() ;
      ~CMIR() {cout << "OBJECT CMIR DESTROYED" << endl ;}
	...
	protected:
      ...
      D_yes_no           md_e_ticket;     // Goes For Irticket
		...
		AB_Boolean             mb_booked_rate_was_overriden_with_dot;
		...
		void populate_Iraddress_SectionA13(AB_Varchar &pv_long_address, 
																CPNR &po_PNR);
		void setShortRemarks(AB_Text pt_remark, AB_Boolean pb_matrix_UDID);
		...
		void getA16ARateData(AB_Text &pt_section, 
		                       ...
		                       V_AB_Text &pd_Data, int pi_Index, AB_Varchar &pv_rateCode, AB_Varchar &pv_rateType,
									  D_char_numeric &pd_comm_rate);
		void getA167RateData(AB_Text &pt_section, 
									  AB_Char &pc_itin_type, 
									  D_char_date &pt_start_date,
		             			  D_saving_code &pd_saving_code, 
									  D_currency &pd_cur_id, 
		...
		void getOverrideRates(AB_Char &pc_itin_type, 
		             					...
		             					D_char_numeric &pd_local_rate, 
											...
											AB_Varchar &pv_rateCode, AB_Varchar &pv_rateType,
											D_char_numeric &pd_comm_rate);
		...
		bool fill_IrOther(AB_Text &pt_section);
		...
		void fill_IrTASF(CPNR &po_PNR);
		void createTASFIrpayment(const AB_Varchar &);
		void createTASFConsumdetail();
		void createTASFRemark();
		void fill_IrOther4TASF(string& sup_id);

		// Service FEE processing
		void processSFC() ;

		// Capture SFC irregularity
		void saveSvcIrregularity() ;
		void getA09XFtaxes(CPNR &po_PNR, AB_Varchar   &pv_XFtaxes);

		void createMCOConsumDetail();
		void createMCOItin();
		AB_Boolean validA04(const AB_Text &a04_seg) const { return a04_seg.length() > 50; }
		void createSFNRemark(AB_Boolean &lb_FR_found);

	   void RP_parseRemarks(CPNR &po_PNR);
		
		//G9139 limo segment
		void fillIrLimo(AB_Text &pt_segment) ;
		void IrLimoRemark( AB_Varchar &pv_field ) ;
		void processNonArcDocFromACCremark();

	   void processEMD(CPNR &po_PNR);
	   void regularOrEquivalentCurAndAmount(AB_Text pt_section, int regCurInd, int regAmountInd, int eqvCurInd, int eqvAmountInd, AB_Varchar& pv_cur, AB_Varchar& pv_amount);
} ;
