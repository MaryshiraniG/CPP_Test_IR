...
(c) Copyright GLOBAL Travel Computer Services 1994, 1995, 1996
      ...
      SID: 1.23
      Delta Created: 16/03/09 - 15:44:51
    Last Delta: app/src/common/InterfaceRecord/_us 3/9/16 - 15:44:51 (/TaskEnv/sw/app/src/common/InterfaceRecord/_us/s.TRMIUR.cpp) 
*/
#pragma comment ( copyright, "@(#)MATRIX{app/src/common/InterfaceRecord/_us}[3/9/16-15:44:51 1.23] /TaskEnv/sw/app/src/common/InterfaceRecord/_us/s.TRMIUR.cpp (unknown)\n" )
/*
...
CTRMIUR::CTRMIUR ()
{
    ...
    LOG(logDEBUG) << "CREATE TRMTRMIUR OBJECT" << endl ;
}

void CTRMIUR::processRMA(D_session_id &pd_sessionId,
							...
							D_branch            &pd_cfaBra) 
{
    ...
    LOG(logDEBUG) << "CTRMIUR::processRMA START...." << endl ;
	...
	createTRMIUR(lo_PNR,pd_cfa_type,pd_cfa_code,pd_cfa_id,
...
//---------------------------------------------------------------------------------------------------------------------------
void CTRMIUR::createTRMIUR(CPNR 				  &po_PNR,
   								D_client_file_type  &pd_cfa_type,
   								D_client_file_code  &pd_cfa_code,
   								D_client_file_no    &pd_cfa_id,
   								D_person_name       &pd_last_name,
   								...
   								D_consultant        &pd_con_id)
{
   ...
   AB_Integer li_sqlcode;
    ...
    LOG(logDEBUG) << "START TRMIUR PROCESSING" << endl ;


	/* REMARK_PARSER: parse ALL remarks */
	RP_parseRemarks(po_PNR);
    LOG(logDEBUG) << " TRMIUR PROCESSING after RP_parseRemarks...." << endl ;

   fill_IrHeader(po_PNR) ;
    LOG(logDEBUG) << " TRMIUR PROCESSING after fill_IrHeader...." << endl ;
...
//--------------------------------------------------------------------------------------------
void CTRMIUR::initGirrawdetail(CGirrawdetailRead  &po_GirrawRead) 
{
...
void CTRMIUR::saveSvcIrregularity()
{
   D_description   ld_errorDescription ;
   AB_Integer      li_msgId ;
